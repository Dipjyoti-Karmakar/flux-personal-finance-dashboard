...
Flux is a **16,000+ line single-file web application**, with all HTML, CSS and JavaScript in one `index.html` file. It uses **Firebase Auth v11.6.1** (Google sign-in popup) and **Cloud Firestore** with persistent local cache for real-time sync across devices and full offline support.
...
| Capability | Details |
| --- | --- |
| **CRUD** | Add, edit, and delete income or expense transactions with category, payment mode (online/offline), date, description, and attached receipts |
...
| Capability | Details |
| --- | --- |
| **Cloud Storage** | Upload multiple receipts (JPG, PNG, WEBP, GIF, PDF) per transaction via Cloudinary unsigned uploads |
| **Inline Viewer** | Dedicated UI to securely view image and PDF receipts directly within the app |
| **Smart Downloads** | Context-aware download logic preserving mobile gestures and enforcing correct file types |
...
The import/export engine uses **SheetJS (xlsx 0.18.5)** for seamless spreadsheet parsing and generation.

| Feature | Details |
| --- | --- |
| **Excel Export** | Generate richly formatted `.xlsx` reports with dynamic column widths, wrapped text, and clickable hyperlinks for uploaded receipts |
| **CSV Import** | Handles auto column-mapping, row validation, and duplicate detection. Writes to Firestore are batched (450 ops/batch) to safely adhere to the 500-write limit |
  ...
  {uid}/
    transactions/             # one doc per transaction
      {txId}
        ...
        receipts    array     (optional) array of objects: { url: string, name: string, type: string }
